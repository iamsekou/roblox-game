...
| Robux slots | **Built 2026-09-24 (Phase 3 item 6).** Game Passes (owner's ids): Sazuki 1995224481, Gozen 1994732512, 2,499 R$ each (`CharacterDefs.gamePassId`). On join `Systems/RobuxService` asks Roblox (`UserOwnsGamePassAsync`, 3 tries); a pass bought in game counts at once (server-side `PromptGamePassPurchaseFinished`). Ownership is never saved (Roblox is the record); a saved Robux *equip* is kept and applied once the pass is confirmed. The shop's BUY WITH ROBUX opens Roblox's prompt |
| Robux credit bundles | **Built 2026-09-24.** Four Developer Products (owner's ids and prices, `Modules/RobuxCatalog`): 5,000 YC 199 R$ (3714587165), 10,000 YC 499 R$ (3714587398), 20,000 YC 999 R$ (3714587623; owner confirmed 20,000, the product is named "2000" on Roblox until renamed), 40,000 YC 1,999 R$ (3714587718). Shown prices are read live from Roblox. In the shop, the balance panel ("+ GET CREDITS") and the "NEED X YC · GET MORE" button open a CREDITS overlay (approved as a tab; built as an overlay). `ProcessReceipt` → `ProfileService.creditPurchase`: the credit and the receipt id are saved in one write before `PurchaseGranted`; a retried receipt already saved is granted without paying again; anything unsaved (profile loading > 45 s, no DataStores, write failed, player gone) answers `NotProcessedYet` so Roblox retries. The newest 100 receipt ids are kept per profile |
...
| IncomeNotice | Event | S→one player | `{source = "passive" or "friends" or "purchase", amount, friends?}` after passive yen is paid or a credit bundle is granted (HUD popup; the shop celebrates a purchase; the balance itself comes through UpdateCurrency) |
...
| M1 | Event | C→S | `()` throw the next punch of the click combo. The server owns the combo step, the target (nearest active enemy within `M1_RANGE`, inside `M1_CONE_DEGREES` of where the puncher faces on the server, line of sight) and the effects; clicks faster than `M1_INTERVAL` are dropped (`Lib/MeleeRules`, `Systems/MeleeService`) |
